...
for z = 1:length(k)
    for i = 1:length(n0)
        for j = 1:length(nb)
                    [PCS,ANS,CPU_TIME,quantile_TIME,optmize_TIME,dele_TIME,sample_TIME]=ASPQ_IS_OptionSA(p,k(z),n0(i),nb(j),delta,mu0,va,trial,c,v,ak,tk,C,alpha,input_S0,input_K,sigma,T,rate,t1);
                    result(count,1) = k(z);
                    result(count,2) = n0(i);
                    result(count,3) = nb(j);
                    result(count,4) = PCS;
