function [PCS, ANS CPU_TIME]=fsp_Sectioning(n0,p,k,nb,delta,va,trial)
% %------自訂-----
% clear;
timer1=tic;
...
trial=trial;%循環測試次數
FileName = ['(nb=',num2str(nb),'p=',num2str(p),')SC_SEC_n0=',num2str(n0),'k=',num2str(k),'var=',num2str(VAR_Y(1)),'del=',num2str(delta),'.mat'];
...
for t = 1:trial
    ...
    %-------刪系統-------
    while true
        [delesys,variance,quan] = dele_system(nb,index,obser,k,r,delta,p,estimate);
        ...
        for i =1 : k
            if index(i) == 0
                obser(i,r+1:r+nb,1) = normrnd(MU_Y(i), VAR_Y(i)^(1/2),[1 nb]);
