function[asym_variance] = asym_var(obs,b,w,s,L,n0,r,p,quant,MU_Y,VAR_Y)
    obs = squeeze(obs);
    ...
    for i = 0: s-1
        if(i == 0)
            ...
            temp2(i+1) = ( ( 1 / ( length(temp1)^2 ) ) * ( (temp1(:,1)>quant)' * temp1(:,2).^2 ) ) - ( ( 1 / length(temp1) ) * (1-p)^2);
        end

    end
    Psi = r * sum(temp2) / (s^2);%%看幾階除掉，意思為把前面的全部一起平均
    %%%%%%%%看要不要加/s^2，跟前面var一樣   
    %if s == 0
    %   Psi = p * (1-p) ;
    %end   
