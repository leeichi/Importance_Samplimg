...
for i = 1:length(n0)
    for j = 1:length(nb)
        for z = 1:length(k)
            ...
            [ANS,PCS,CPU_TIME,quantile_TIME,optmize_TIME,dele_TIME,sample_TIME]=fsp_IS_OptionSAA(n0(i),p,k(z),nb(j),delta,mu,va,trial,input_S0,input_K,sigma,T,rate,t1,amount);
            result(count,1) = k(z);
            result(count,2) = n0(i);
            result(count,3) = nb(j);
            result(count,4) = PCS;
